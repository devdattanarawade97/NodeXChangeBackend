...
router.post('/run-thread', async (req, res) => {
    ...
    let response = "";
    const { userId, threadId, isGPTResponse, message, relevantText } = req.body;
    ...
    try {
        ...
        await addMessageToThread(threadId, isGPTResponse, message, relevantText); //this is gpt model
...
router.post('/create-thread-and-run', async (req, res) => {
    ...
    let response = "";
    const { message , relevantText } = req.body;

    try {
        //log message withing create thread and run
        console.log("msg within create thread and run endpoint   : ", message);

        const runResponse = await createAndRunThread(message , relevantText); //this is gpt model
